...
CREATE TABLE F15C4_Rfe
  (
    ...
    stat_id      INTEGER NOT NULL ,
    emp_id       INTEGER NOT NULL ,
    lab_dir_id   INTEGER NOT NULL ,
    exe_dir_id   INTEGER NOT NULL ,
    sys_admin_id INTEGER NOT NULL ,
    chair_id     INTEGER
  ) ;
...
ALTER TABLE F15C4_Stat ADD CONSTRAINT r12 FOREIGN KEY ( hist_id ) REFERENCES F15C4_Hist ( hist_id ) ;
...
CREATE OR REPLACE TRIGGER FKNTM_F15C4_Auth BEFORE
  UPDATE OF emp_id ON F15C4_Auth BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Auth is violated');
END;
/

CREATE OR REPLACE TRIGGER FKNTM_F15C4_Doc BEFORE
  UPDATE OF emp_id,
    rfe_id ON F15C4_Doc BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Doc is violated');
END;
/

CREATE OR REPLACE TRIGGER FKNTM_F15C4_Hist BEFORE
  UPDATE OF stat_id ON F15C4_Hist BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Hist is violated');
END;
/
...
CREATE OR REPLACE TRIGGER FKNTM_F15C4_Rfe BEFORE
  UPDATE OF emp_id,
    lab_dir_id,
    exe_dir_id,
    sys_admin_id ON F15C4_Rfe BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Rfe is violated');
END;
/

CREATE OR REPLACE TRIGGER FKNTM_F15C4_Task BEFORE
  UPDATE OF rfe_id ON F15C4_Task BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Task is violated');
END;
/

CREATE OR REPLACE TRIGGER FKNTM_F15C4_Emp BEFORE
  UPDATE OF lab_id,
    auth_id ON F15C4_Emp BEGIN raise_application_error(-20225,'Non Transferable FK constraint  on table F15C4_Emp is violated');
END;
/
